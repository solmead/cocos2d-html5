import { AffineTransform, Rect } from "../cocoa/index";


var regionPool: Array<Region> = [];
...
function regionRelease(region:Region) {
...
export class Region {
    ...
    updateRegion(bounds: Rect, matrix:AffineTransform) {
...
export class DirtyRegion {
    ...
    setClipRect(width:number, height:number) {
    ...
    mergeDirtyList(dirtyList: Array<Region>): boolean {
        var length = dirtyList.length;
        if (length < 2) {
            return false;
        }
        ...
        for (var i = 0; i < length - 1; i++) {
            ...
            hasClipRect && (totalArea += regionA._area);
            for (var j = i + 1; j < length; j++) {
                var regionB = dirtyList[j];
                var delta = unionArea(regionA, regionB) - regionA._area - regionB._area;
                if (bestDelta > delta) {
                    mergeA = i;
                    mergeB = j;
                    bestDelta = delta;
