import { ccClass, REPEAT } from "../platform/index";
import { Size, size, rect, Rect } from "../cocoa/index";
import { Texture2D, UIImage } from "./CCTexture2D";
...
export class Texture2DCanvas extends Texture2D {
    ...
    _grayElementObj: UIImage = null;
    _backupElement: UIImage = null;
    _isGray: boolean = false;

    channelCache = new Array<UIImage>();
    ...
    _generateColorTexture(r: number = null, g: number = null, b: number = null, irect: Rect = null, canvas: UIImage = null): Texture2D | HTMLCanvasElement {/*overide*/
    ...
    _generateColorTextureNewBlend(r: number = null, g: number = null, b: number = null, irect: Rect = null, canvas: UIImage = null): Texture2D | HTMLCanvasElement {/*overide*/
    ...
    _generateColorTextureOld(r: number = null, g: number = null, b: number = null, irect: Rect = null, canvas: UIImage = null): Texture2D | HTMLCanvasElement {/*overide*/
        ...
        if (!irect)
            irect = rect(0, 0, textureImage.width, textureImage.height);
        var x, y, w, h;
        x = irect.x; y = irect.y; w = irect.width; h = irect.height;
        if (!w || !h)
            return;
    ...
    _generateTextureCacheForColor(): Array<UIImage> {
        if (this.channelCache)
            return this.channelCache;

        var textureCache = <Array<UIImage>>[
...
var renderToCache = function (image: HTMLCanvasElement | HTMLImageElement, cache: Array<UIImage>) {
    var w = image.width;
    var h = image.height;
    ...
    var cacheCtx = (<HTMLCanvasElement>cache[3]).getContext("2d");
    cacheCtx.drawImage(image, 0, 0);
    var pixels = cacheCtx.getImageData(0, 0, w, h).data;

    var ctx;
    for (var rgbI = 0; rgbI < 4; rgbI++) {
        ctx = (<HTMLCanvasElement>cache[rgbI]).getContext("2d");
...
function _generateGrayTexture(texture: HTMLCanvasElement | HTMLImageElement, irect: Rect = null, renderCanvas: HTMLCanvasElement = null): HTMLCanvasElement {
    if (texture === null)
        return null;
